...
export type PlanUsageEvent = {
  ...
  rateLimits: {
     ...
     * Anthropic's own percentage rate-limit windows above, and distinct from
     * Claudius's own client-enforced `LimitsPanel` caps). Optional because,
     * as of the SDK version this repo currently builds against, the upstream
     * `usage_EXPERIMENTAL_MAY_CHANGE_DO_NOT_RELY_ON_THIS_API_YET()` response
     * does not publish a `spend_limit` field yet (checked against the latest
     * published `@anthropic-ai/claude-agent-sdk` `.d.ts` at review time) — the
     * field names below are our best-effort guess at the eventual shape,
     * mirroring the sibling `extra_usage` object's `{limit,used,utilization}`
     * pattern. Renders only when present; absent is the expected case today.
     */
    spendLimit?: {
