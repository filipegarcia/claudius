 ...
 * `app/[workspaceId]/permissions/`; landing here means the cookie was
 * unreadable when middleware ran (first boot, cleared cookies). The
 * helper resolves the active workspace via the store hint and
 * 307s the browser to the canonical `/<id>/permissions` URL, preserving any
...
export default async function Page({
  ...
  await redirectToWorkspaceRoute("/permissions", sp);
}
