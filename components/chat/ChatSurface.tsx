...
/**
 * The chat surface — shared by the workspace chat route (`/[workspaceId]`) and
 * the customization chat route (`/customize/[id]/chat`). The `kind`/`id`/`cwd`
 * context props replace the old `useParams<{workspaceId}>` identity read so the
 * same component serves both. For a customization there is no Workspace record,
 * so `cwd` carries the mirror src dir (used for the foreign-session filters) and
 * `activeWorkspace` stays null.
 */
export type ChatSurfaceContext = {
  kind: "workspace" | "customization";
  /** workspace id or customization id, depending on `kind`. */
  id: string;
  /** Customization mirror src dir; null for a workspace (derived from the store). */
  cwd: string | null;
};

export default function ChatSurface({ kind, id: contextId, cwd: contextCwd }: ChatSurfaceContext) {
  // For a customization, pass the mirror src dir so fresh sessions land there
  // without depending on the active-customization cookie. Workspaces pass null
  // and let the server resolve cwd from the active-workspace cookie.
  const session = useSession({ defaultCwd: kind === "customization" ? contextCwd : null });
  ...
  // Identity comes from the context props (set by the route wrapper), not the
  // URL params — so this component serves both /[workspaceId] and
  // /customize/[id]/chat. For a customization there is no Workspace record.
  const activeWorkspaceId = kind === "workspace" ? contextId : null;
  const activeWorkspace =
    workspaceItems.find((w) => w.id === activeWorkspaceId) ?? null;
  // The root dir used for foreign-session filtering. A workspace uses its
  // rootPath; a customization uses the mirror src dir passed in `cwd`.
  const contextRoot = kind === "customization" ? contextCwd : (activeWorkspace?.rootPath ?? null);
  ...
  const verbose = useVerbose(contextId);
  ...
  useEffect(() => {
    ...
    const root = contextRoot ?? undefined;
  ...
  return (
    <div className="flex h-full">
      ...
      <main data-pane-name="chat-area" className="relative flex h-full min-w-0 flex-1 flex-col">
        ...
        />
        <div data-pane-name="message-list" className="flex flex-1 flex-col overflow-hidden">
          {searchOpen && (
            <TranscriptSearch
              sessionId={session.sessionId}
