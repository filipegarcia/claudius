#!/usr/bin/env python3
"""
Generate the release-analytics pages from release_data.json
Run from claudius/scripts/releases/:
    python3 generate_dashboard.py                        # multi-repo dashboard
    FOCUS_REPO="Claude Code" python3 generate_dashboard.py   # focus page
Reads:  scripts/releases/release_data.json
Writes: site/releases.html            (default)
        site/releases-claude-code.html (FOCUS_REPO="Claude Code")

Both pages are regenerated and committed by .github/workflows/releases.yml;
adding a new focus page means adding another FOCUS_REPO run there too.
"""
...
if FOCUSED:
    ...
    # The Claude Code focus page is generated by a second FOCUS_REPO run (see
    # .github/workflows/releases.yml). Link to it so the dashboard is the entry
    # point rather than the focus page being an orphan only reachable by URL.
    BREADCRUMB  = ('<a href="/releases.html">Anthropic Releases</a>\n'
                   '      <span>&middot;</span>\n'
                   '      <a class="nav-focus" href="/releases-claude-code.html">'
                   'Claude Code deep dive &rarr;</a>')
...
<style>
  *, *::before, *::after {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  /* Cross-link out of the multi-repo dashboard into a focus page. Not a
     breadcrumb ancestor, so it gets the accent colour and its own separator. */
  .header-nav a.nav-focus {{ color: #d97757; }}
  .header-nav a.nav-focus:hover {{ color: #e8a98d; }}
  header h1 {{
